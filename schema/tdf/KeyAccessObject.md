...
|`kid`|String|Identifier for the KAS public key, such as its thumbprint. The current prefferred identifier can be looked up using the `kas_public_key` endpoint. For compatibility, our reference implementation uses the associated x509 certificate's fingerprint.|Recommended|
