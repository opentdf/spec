...
3.  **KAS Verification:** The KAS authenticates the client, decrypts the wrapped DEK share(s) using its private key, validates the policy binding against the provided policy and the decrypted DEK share, and requests an authorization decision from the Policy Decision Point (PDP), which evaluates the policy against the client's authenticated attributes.
